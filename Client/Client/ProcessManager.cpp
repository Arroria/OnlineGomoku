...
WPARAM ProcessManager::Loop()
{
	...
	FrameTimer::time_point prevRenderTime;
	FrameTimer::time_point currTime;
	...
	while (msg.message != WM_QUIT)
	{
		...
		{
			currTime = FrameTimer::now();

			//Update
			if (m_update)
			{
				m_updateDeltaTime = currTime - prevUpdateTime;
				if (!m_updateFPSLimite || m_updateDeltaTime * m_updateFPSLimite >= std::chrono::seconds(1))
				{
